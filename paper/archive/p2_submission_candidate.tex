...
($\mathrm{STER}{=}0.0000$), and still violate timing constraints under
deployment load---these are structurally independent properties that
current pre-market validation does not distinguish.
This letter demonstrates that these properties are independent through a controlled
...
activations, both paths maintain $\mathrm{STER}{=}0.0000$, confirming
output instability is not the dominant failure mode.
...
by 65\%.
The Safety-Threshold Exceedance Rate (STER) and latency verification
are proposed jointly as a candidate pre-market acceptance protocol
under FDA Draft Guidance FDA-2024-D-4488 (January 2025).
\end{abstract}
...
however, to our knowledge, no standardized empirical method exists for characterizing
whether inference pipeline timing is preserved under realistic deployment
stress.
Current pre-market validation frameworks can certify a system as
functionally correct while it simultaneously violates real-time
operational constraints under deployment load---a failure mode that is
structurally invisible to accuracy-based testing.

This letter is not a performance optimization study; it exposes a
validation failure: standard accuracy-based certification cannot detect
operational constraint violations that occur under deployment load.
The novelty is not timing degradation---that is known---but the
validation blindness to it.
The safety-relevant threshold is not latency magnitude but latency
relative to the operational deadline at a fixed clinical activation rate.
This letter addresses this gap through a controlled experiment in which
all hardware variables are held constant, so any observed divergence
arises from execution context---isolating the software execution path
as the operative factor.
This letter demonstrates that accuracy-based validation can certify
systems that violate timing constraints under deployment load, and
introduces STER to isolate output stability from temporal safety---two
properties current validation does not distinguish.
This work does not study timing degradation itself; it exposes a
validation failure where such degradation remains undetected under
accuracy-based certification ($\Delta\mathrm{acc}{<}1.0\%$ on both paths).
The controlled same-hardware experiment shows STER\,=\,0.0000 on both
paths while CPU-path latency degrades $7.2{\times}$ to
P99\,=\,165.1\,ms under combined load; a joint STER\,+\,latency
pre-market protocol operationalizing FDA-2024-D-4488 is proposed.
...
Prior work optimizes or characterizes inference performance and isolation
within GPU execution; this letter demonstrates a validation-level failure
that is independent of performance optimization---the contribution is
problem class separation, not methodological novelty alone~\cite{martin2026}.
...
\begin{equation}
  ...
  \label{eq:ster}
\end{equation}
Accuracy testing evaluates $\mathrm{argmax}(\sigma(\mathbf{y}))$---a
single class label---and cannot detect bounded distribution drift below
the argmax threshold; this is a structural limitation, not a sensitivity issue.
A model whose top-1 prediction is unchanged but whose confidence
distribution has shifted by 4.9\% passes accuracy testing.
STER captures this class of silent deviations; accuracy cannot by construction.
STER is proposed as a candidate pre-market metric, not an existing
FDA-specified standard.
This threshold reflects conservative tolerance bounds relative to
ISO~14971 Table~B.1 risk control examples, which typically reference
10--20\% functional deviation thresholds.
...
10\,Hz is a representative real-time clinical monitoring constraint,
not a universal SaMD requirement; appropriate activation rates are
...
A timing violation is defined as deviation beyond a threshold that
compromises deadline adherence at a fixed clinical activation rate,
where missed deadlines preclude synchronous data delivery to downstream
decision processes.
The CPU-path P99 of 165.1\,ms constitutes such a violation at 10\,Hz:
1\% of inferences miss the cycle budget by ${\geq}65\%$, and this
violation persists undetected under accuracy-based validation
($\Delta\mathrm{acc}{<}1.0\%$).
Such deviations are incompatible with systems requiring bounded
execution intervals; they can propagate to downstream decision
pipelines, invalidating synchronization assumptions and compromising
...
1024-core Ampere GPU via dedicated Direct Memory Access (DMA) paths with Quality of Service (QoS) bandwidth
...
Jetson surface temperature was monitored via tegrastats throughout all
trials; no thermal throttling events were recorded and sustained clock
frequencies were confirmed across all conditions.
...
All CPU inferences used CPUExecutionProvider in deterministic mode
with a fixed thread count and no graph optimizations that introduce
nondeterminism.
Trial counts ($N{=}500$ per condition, 10 trials per stressor level)
were selected to provide stable mean and P99 latency estimates; the
$7.2{\times}$ divergence represents a practically significant difference
independent of formal hypothesis testing.
...
timing diverges significantly: the GPU path is invariant
(${\approx}14.8$\,ms regardless of load) while the CPU path
reaches $73.7$\,ms at 75\% load ($5.1{\times}$ baseline).
TensorRT FP16 executes on the GPU, isolated from saturated CPU cores;
ONNX Runtime FP32 competes for CPU time slices and cache bandwidth,
degrading latency without affecting arithmetic output.
...
GPU DMA isolation, QoS bandwidth reservation, and independent TensorRT
contexts prevent all stressor classes from perturbing outputs or latency;
a confirmed +88\% interrupt rate at BLE conns\,=\,4 had no effect.
...
The observed timing divergence is not itself the contribution; the
contribution is that this divergence remains undetected under standard
accuracy-based validation despite violating operational constraints---a
system passes certification while being undeployable at the required
activation rate.

% ── VI. Discussion ────────────────────────────────────────────────────────────
\section{Discussion}

The contribution is not the presence of timing variation under load---that
is expected system behavior---but the structural inability of existing
accuracy-based validation to detect it, which this controlled experiment
...
These results constitute engineering-level evidence for a safety-relevant
validation primitive, not a complete clinical protocol; further validation
against regulated device workflows is required before 510(k) adoption.
Running GPU and CPU paths on the same Jetson Orin Nano Super controls
for all hardware-level confounds; timing divergence of $7.2{\times}$
under identical stressors is consistent with a causal interpretation
attributing divergence to the accelerator architecture, as cross-platform
comparisons would leave hardware differences as alternative explanations.
Platform datasheets document architectural separation in principle;
this work provides empirical quantification at clinical deployment intensities.

\subsection{Timing as the Dominant Failure Mode}

STER\,=\,0.0000 on both paths establishes that output instability is
not the failure mode; the dominant failure mode is timing degradation,
invisible to STER, accuracy, and any output-only evaluation method.
This motivates timing characterization as a complementary pre-market
verification property.

\subsection{Pre-Market Validation Gap}

A manufacturer using accuracy-only testing would clear both
execution paths as equivalent; only the joint condition~(\ref{eq:accept})
distinguishes them, as $\Delta\mathrm{acc}{<}1.0\%$ on both paths while
the CPU-only path breaches the 10\,Hz cycle budget by 65\%.

\subsection{STER as Output Stability Certification}

STER certifies the full output distribution $\sigma(\mathbf{y})$
within $T^*$---a property accuracy ($\mathrm{argmax}(\sigma(\mathbf{y}))$)
structurally cannot confirm---thereby isolating timing as the sole
safety-relevant divergence under all tested conditions.

\subsection{Limitations}

Results are specific to MobileNetV2 at 10\,Hz on Jetson Orin Nano Super
under JetPack~6; models with higher memory bandwidth utilization or
non-deterministic preprocessing may exhibit different behavior.
$T^*{=}0.05$ is conservative relative to ISO~14971 Table~B.1 but is
not clinically validated for any specific device class.
The GPU and CPU paths differ in execution hardware and runtime stack
(TensorRT vs.\ ONNX Runtime); future work isolating a single runtime
would further strengthen the attribution claim.
This study characterizes inference-layer timing and output stability only
and does not evaluate preprocessing latency, real-time OS scheduling,
...
STER provides a mechanism to verify output distribution bounds; latency
verification under contention is required for timing safety.
Output correctness alone is insufficient for reliable clinical deployment;
temporal behavior under deployment contention must be explicitly validated.
Joint STER\,+\,latency verification is proposed, operationalizing
